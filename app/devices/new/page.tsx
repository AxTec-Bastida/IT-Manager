...
export default async function NewDevicePage() {
  ...
  const [ranges, settings, employees, facturas, zones, devices] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        <DeviceForm ranges={ranges} employees={employees} facturas={facturas} zones={zones} defaults={{ vlan: settings.defaultVlan, category: settings.defaultCategory }} devices={devices} />
      </div>
    </div>
  );
}
